...
regex regular(".*places.sqlite");
vector<string> paths;
cmatch result;
GetFile(paths, Path);
	for (auto i : paths)
		if (regex_match(i.c_str(), result, regular)) Path = i;
...
{
sqlite3_prepare_v2(DB,"select * from moz_places;",-1,&State,NULL);
...
{
int result = sqlite3_exec(DB,"delete from moz_places;",NULL,NULL,&errmsg);
VirtualStringTree1->Clear();
}
//---------------------------------------------------------------------------
...
{
TreeData * treeData =(TreeData*)VirtualStringTree1->GetNodeData(Node);
std::string sql = "delete from moz_places where id =" + std::to_string(treeData->id) + ";";
...
auto node = Sender->GetFirstSelected();
	TreeData * treeData = (TreeData*)Sender->GetNodeData(node);
	std::string sql = "SELECT datetime((last_visit_date/1000000),'unixepoch', 'utc') AS time FROM moz_places where id ="+ std::to_string(treeData->id) + ";";
